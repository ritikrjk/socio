...
const authMiddleware = async (req, res, next) => {
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return res.status(401).json({ message: "Authentication error: No token provided or malformed token." });
  }
  ...
  try {
    ...
    if (!user) {
      // This case is important for when a user has been deleted but their token is still valid.
    ...
    if (error.name === "JsonWebTokenError") {
      return res.status(401).json({ message: "Token is invalid. Please re-authenticate." });
    }

    // Fallback for any other unexpected errors during token processing
